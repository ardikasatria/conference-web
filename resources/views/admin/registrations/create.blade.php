...
<div class="container-fluid">
    @include('layouts.partials.page-title', ['title' => 'Create Registration', 'subtitle' => 'Add a new registration'])
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.registrations.store') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="payment_status" class="form-select @error('payment_status') is-invalid @enderror">
                                @foreach(['unpaid', 'partial', 'paid', 'refunded'] as $s)
                                    <option value="{{ $s }}" {{ old('payment_status', 'unpaid') === $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                                @endforeach
                            </select>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary"><i class="ti ti-check me-1"></i> Create Registration</button>
